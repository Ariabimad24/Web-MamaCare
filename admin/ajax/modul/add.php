...
// Helper function to generate modul file name
function generateModulFileName($judul) {
    return strtolower(str_replace(" ", "-", $judul)) . ".php";
}

// Helper function to generate modul content
...
// Helper function to generate modul card
...
if (isset($_POST['submit'])) {
    try {
        ...
        // Generate file name for the modul
        $modulFileName = generateModulFileName($judul);
        $modulFilePath = "../../../modul/" . $modulFileName;

        // Database insertion with file path
        $sql = "INSERT INTO modul_kesehatan (judul, isi, gambar, highlight, file_path) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            throw new Exception("Prepare failed: " . $conn->error);
        }

        $stmt->bind_param("sssss", $judul, $isi, $gambar, $highlight, $modulFileName);

        if ($stmt->execute()) {
            // Generate and save modul content
            $modulContent = generateModulContent($judul, $gambar, $highlights, $isis);
            if (!file_put_contents($modulFilePath, $modulContent)) {
                throw new Exception("Gagal membuat file modul");
            }
            chmod($modulFilePath, 0644);

            // Update all-modul.php
            $allModulPath = "../../../all-modul.php";
            $content = file_get_contents($allModulPath);
            
            $cardContent = "\n        " . generateModulCard($modulFileName, $gambar, $judul);

            $gridStart = strpos($content, '<div class="modul-grid">');
            
            if ($gridStart !== false) {
                $insertPosition = $gridStart + strlen('<div class="modul-grid">');
                $newContent = substr($content, 0, $insertPosition) . $cardContent . substr($content, $insertPosition);
                
                if (!file_put_contents($allModulPath, $newContent)) {
                    throw new Exception("Gagal memperbarui all-modul.php");
                }
            } else {
                throw new Exception("Tidak dapat menemukan section modul-grid");
            }
...
<body class="hold-transition sidebar-mini">
    ...
    <div class="content-wrapper">
        <h2>Form Tambah Modul Kesehatan</h2>
        <div class="form-container">
            <form action="add.php" method="POST" enctype="multipart/form-data">
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <tr>
                            <td><label for="judul">Judul Modul:</label></td>
                            <td><input type="text" id="judul" name="judul" class="form-control" required></td>
                        </tr>
                        <tr>
                            <td><label for="gambar">Gambar Modul:</label></td>
...
<br>
                <button type="submit" name="submit" class="btn btn-primary">Tambah Modul</button>
            </form>
        </div>
    </div>
