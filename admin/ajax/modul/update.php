...
function generateModulFileName($judul) {
    return strtolower(str_replace(" ", "-", $judul)) . ".php";
}

function generateModulContent($judul, $gambar, $highlights, $isis) {
    return '<?php
session_start();
...
function generateModulCard($fileName, $gambar, $judul) {
    return '<a href="modul/' . $fileName . '" style="text-decoration: none; color: inherit;">
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $conn->prepare("SELECT * FROM modul_kesehatan WHERE id = ?");
    ...
    if (!$modul) {
        echo "<script>alert('Modul tidak ditemukan.'); window.location.href = '../modul-admin.php';</script>";
        exit;
    }
...
if (isset($_POST['update'])) {
    try {
        ...
        // Get old modul information
        $stmt = $conn->prepare("SELECT gambar, file_path FROM modul_kesehatan WHERE id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $result = $stmt->get_result();
        $old_modul = $result->fetch_assoc();
        $old_file_path = $old_modul['file_path'];
        $gambar = $old_modul['gambar'];
        ...
        if (isset($_FILES['gambar']) && $_FILES['gambar']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($old_modul['gambar'] && file_exists('../../../' . $old_modul['gambar'])) {
                unlink('../../../' . $old_modul['gambar']);
            }
        ...
        $new_file_path = ($judul !== $old_judul) ? generateModulFileName($judul) : $old_file_path;

        // Update database first
        $sql = "UPDATE modul_kesehatan SET judul = ?, isi = ?, gambar = ?, highlight = ?, file_path = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssi", $judul, $isi, $gambar, $highlight, $new_file_path, $id);

        if ($stmt->execute()) {
            // Generate new modul content
            $modulContent = generateModulContent($judul, $gambar, $highlights, $isis);
            
            // Handle file updates
            $old_modul_path = "../../../modul/" . $old_file_path;
            $new_modul_path = "../../../modul/" . $new_file_path;

            // If the file name is changing, we need to create new file and delete old one
            if ($new_file_path !== $old_file_path) {
                // Delete old file if it exists
                if (file_exists($old_modul_path)) {
                    unlink($old_modul_path);
                }
            }

            // Write new content to file
            if (file_put_contents($new_modul_path, $modulContent) === false) {
                throw new Exception("Gagal memperbarui file modul: " . error_get_last()['message']);
            }
            
            // Set proper permissions
            chmod($new_modul_path, 0644);

            // Update all-modul.php
            $allModulPath = "../../../all-modul.php";
            if (file_exists($allModulPath)) {
                // Read the current content
                $content = file_get_contents($allModulPath);
                if ($content === false) {
                    throw new Exception("Gagal membaca all-modul.php");
                }

                // Generate old and new cards
                $old_card = generateModulCard($old_file_path, $old_modul['gambar'], $old_judul);
                $new_card = generateModulCard($new_file_path, $gambar, $judul);

                // Replace the old card with the new one
                $updated_content = str_replace($old_card, $new_card, $content);
                
                // Write the updated content back to the file
                if (file_put_contents($allModulPath, $updated_content) === false) {
                    throw new Exception("Gagal memperbarui all-modul.php");
                }
            }
...
<body class="hold-transition sidebar-mini">
    ...
    <div class="form-container">
        ...
        <form method="POST" enctype="multipart/form-data">
            <input type="hidden" name="modul_id" value="<?php echo isset($modul) ? $modul['id'] : ''; ?>">
            <input type="hidden" name="old_judul" value="<?php echo isset($modul) ? $modul['judul'] : ''; ?>">
            
            <div class="form-group">
                <label for="judul">Judul Modul:</label>
                <input type="text" id="judul" name="judul" value="<?php echo isset($modul) ? htmlspecialchars($modul['judul']) : ''; ?>" required>
            </div>

            <div class="form-group">
                <label for="gambar">Gambar Modul (Biarkan kosong jika tidak ingin mengubah):</label>
                <input type="file" id="gambar" name="gambar" accept="image/*">
                <?php if (isset($modul) && $modul['gambar']): ?>
                    <img src="../../../<?php echo $modul['gambar']; ?>" alt="Current Image" style="max-width: 200px; margin-top: 10px;">
                <?php endif; ?>
            </div>
            ...
            <button type="submit" name="update">Update Modul</button>
        </form>
    </div>
    <script>
