...
// Validasi jika sertifikat_kedokteran memiliki nilai
if (!empty($file_path)) {
    // Pastikan path dimulai dengan 'uploads/sertifikat/'
    if (!str_contains($file_path, 'uploads/sertifikat/')) {
        $file_path = 'uploads/sertifikat/' . $file_path;
    }

    // Tampilkan link hanya jika file_path valid
    echo "<td class='align-middle text-center'>
            <a href='" . $base_url . htmlspecialchars($file_path) . "' target='_blank' style='text-decoration: none;'>
                <span style='display: block; text-align: center; color: blue; font-size: 15px;'>Link Sertifikat</span>
            </a>
          </td>";
} else {
    // Jika tidak ada sertifikat, tampilkan placeholder
    echo "<td class='align-middle text-center'>
