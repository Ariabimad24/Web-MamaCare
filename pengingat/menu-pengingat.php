...
if ($user_type == 'ibu_muda') {
    ...
    $query = "SELECT id, waktuPengingat, instruksi_arahan_dokter, status FROM pengingat_jadwal_kesehatan WHERE ibu_muda_id = ? ORDER BY waktuPengingat ASC";
    ...
    $photo_profile = $user['photo_profile'];
    $user_name = $user['nama'];
    $pengingat_list = []; // Nakes tidak memiliki pengingat
...
<div id="popupOverlay" class="overlay" style="display: none;">
    ...
    <table class="pengingat-table">
        ...
        <?php if (empty($pengingat_list)): ?>
                    <tr>
                        <td colspan="5" class="no-data">Belum ada pengingat jadwal kesehatan</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($pengingat_list as $pengingat): ?>
                        <tr data-id="<?php echo $pengingat['id']; ?>" onclick="showPopup(<?php echo $pengingat['id']; ?>, '<?php echo htmlspecialchars($pengingat['instruksi_arahan_dokter']); ?>')">
                            ...
                            <td><?php echo $pengingat['status']; ?></td>
                            <td>
                                <a href="edit_reminder.php?id=<?php echo $pengingat['id']; ?>" class="btn btn-edit">Edit</a>
                                <a href="delete_reminder.php?id=<?php echo $pengingat['id']; ?>" class="btn btn-delete" onclick="return confirm('Apakah Anda yakin ingin menghapus pengingat ini?')">Hapus</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
